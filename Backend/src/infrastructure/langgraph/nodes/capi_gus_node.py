...
DescripciÃ³n: Nodo responsable de entregar la respuesta final al usuario en tono amigable.
...
class CapiGusNode(GraphNode):
    ...
    def __init__(self, name: str = "capi_gus", reasoner: Optional[LLMReasoner] = None) -> None:
        ...
        self._greeting_responses = [
            ...
            "Â¡Buen dÃ­a! Â¿Te gustarÃ­a un resumen financiero o revisamos alguna sucursal?",

            "Â¡Hola! Estoy listo para ayudarte con tus anÃ¡lisis.",

            "Â¡Saludos! Â¿Quieres que revisemos saldos, sucursales o anomalÃ­as?",

        ]

        self._conversation_responses = [

            "Siempre es bueno conversar; si querÃ©s puedo preparar un resumen o revisar una sucursal.",

            "Estoy a tu disposiciÃ³n para analizar datos financieros cuando lo necesites.",

            "Gracias por el mensaje. Â¿Te ayudo con un resumen, un anÃ¡lisis de sucursal o detecciÃ³n de anomalÃ­as?",

            "Encantado de seguir charlando, pero tambiÃ©n puedo generar reportes o detectar anomalÃ­as.",
    ...
    def _handle_conversation(self, state: GraphState) -> Tuple[str, Dict[str, Any]]:
        ...
        if any(token in query for token in self._vdg_keywords):
            ...
            response = (
                ...
                "Los Capi Agentes son el equipo mÃ¡s vibrante de Viernes de Garage: "

                "mezclamos datos, calle y pura onda para levantar cualquier demo y enamorar al pÃºblico."
            ...
            response = "Â¡De nada! Â¿QuerÃ©s que revisemos otra sucursal o prepare un informe?"
    ...
    def _compose_response(self, state: GraphState) -> Tuple[str, Dict[str, Any], Optional[Dict[str, Any]]]:
        ...
        def _build_gmail_confirmation(recipient_list: list[str], subject_line: str) -> Tuple[str, Dict[str, Any]]:
            ...
            message_text = f"Te confirmo que enviÃ© el correo a {joined} con asunto \"{subject_text}\". Â¿NecesitÃ¡s algo mÃ¡s?"
    ...
    def _compose_global_response(
        ...
        policy_info = summary.get("policy") or {}
        tolerance_text: Optional[str] = None
        tolerance_pct_value: Optional[Decimal] = None
        tolerance_raw = self._to_decimal(policy_info.get("max_surplus_pct"))
        if tolerance_raw and tolerance_raw > 0:
            tolerance_pct_value = tolerance_raw * Decimal("100")
            if tolerance_pct_value == tolerance_pct_value.to_integral():
                tolerance_text = f"{int(tolerance_pct_value)}%"
            else:
                tolerance_text = f"{tolerance_pct_value.quantize(Decimal('0.1'))}%"
        ...
        if total_branches > 0:
            if tolerance_text:
                first_sentence = (
                    f"Analice {total_branches} sucursales contra la franja tolerable del canal Saldo Total ({tolerance_text})."
                )
            else:
                first_sentence = f"Analice {total_branches} sucursales."
        else:
            first_sentence = "Analice las sucursales disponibles."
        ...
        closing_question = "Quieres la informacion mas detallada en un Excel?"

        base_message = " ".join(
            sentence.strip() for sentence in (first_sentence, second_sentence, action_sentence) if sentence
        ).strip()
        final_message = f"{base_message} {closing_question}".strip()
        ...
        context_snapshot = {
            ...
            "surplus_amount": self._decimal_to_float(surplus_amount),


            "surplus_amount_formatted": self._format_currency(surplus_amount) if surplus_amount else "0",


            "deficit_count": deficit_count,


            "deficit_amount": self._decimal_to_float(deficit_amount),


            "deficit_amount_formatted": self._format_currency(deficit_amount) if deficit_amount else "0",


            "tolerance_pct": float(tolerance_pct_value) if tolerance_pct_value is not None else None,
        ...
        # Para mantener el tono solicitado de manera consistente evitamos delegarlo al LLM.
        ...
        if isinstance(datab_bucket, dict):
            ...
            export_file = databucket_export = datab_bucket.get("export_file")
    ...
    def _generate_global_summary_with_llm(
        ...
        *,
        ...
        """Generate executive summary in natural language for global branch analysis."""
        ...
        surplus_count = context_snapshot.get("surplus_count", 0)

        deficit_count = context_snapshot.get("deficit_count", 0)

        total_branches = context_snapshot.get("total_branches", 0)

        surplus_amount_fmt = context_snapshot.get("surplus_amount_formatted", "$0,00")

        deficit_amount_fmt = context_snapshot.get("deficit_amount_formatted", "$0,00")



        system_prompt = (

            "Sos Capi Gus, el agente ejecutivo de CapiAgentes. "

            "Tu especialidad es resumir anÃ¡lisis financieros complejos en lenguaje natural claro y profesional. "

            "RespondÃ© en castellano rioplatense con tono ejecutivo pero cercano. "

            "UsÃ¡ la informaciÃ³n financiera para construir un resumen narrativo que cuente la historia de los datos."
        ...
        query = (
            ...
            f"Acabamos de analizar {total_branches} sucursales y necesito que generes un resumen ejecutivo en lenguaje natural. "

            f"Encontramos {surplus_count} sucursales con exceso de efectivo ({surplus_amount_fmt}) "

            f"y {deficit_count} sucursales con dÃ©ficit ({deficit_amount_fmt}). "

            "El resumen debe: "

            "(1) Empezar diciendo que completaste el anÃ¡lisis de todas las sucursales, "

            f"(2) Mencionar cuÃ¡ntas tienen exceso ({surplus_count}) y cuÃ¡nto dinero en total necesitan retirar ({surplus_amount_fmt}), "

            f"(3) Mencionar cuÃ¡ntas tienen dÃ©ficit ({deficit_count}) y cuÃ¡nto dinero total necesitan depositar ({deficit_amount_fmt}), "

            "(4) Dar una recomendaciÃ³n de equilibrio (ej: 'Para equilibrar las sucursales, necesitÃ¡s depositar X y retirar Y'), "

            f"(5) Cerrar con: '{closing_question}'. "

            "MÃ¡ximo 4-5 oraciones. Tono profesional y directo como un CFO reportando a la gerencia."
        ...
        try:
            ...
            try:
                ...
                logger.warning({"event": "capi_gus_global_llm_failed", "trace_id": trace_id, "error": str(exc)})
            ...
            if llm_result.error:
                ...
                logger.warning(

                    {
                        ...
                        "event": "capi_gus_global_llm_error",
        ...
        logger.info(

            {
                ...
                "event": "capi_gus_global_llm_completed",
