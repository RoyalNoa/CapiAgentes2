    ...
    async def process(self, task) -> AgentResult:
        ...
        agent_progress.start(
            ...
            extra={
                ...
                'operation': (task.metadata or {}).get('operation') if isinstance(getattr(task, 'metadata', None), dict) else None
            }
        )
        ...
        status_flag = getattr(result, 'status', None)
        if success_flag is None and status_flag is not None:
            success_flag = str(status_flag).lower() in {'completed', 'success', 'ok', 'true'}
        if bool(success_flag):
            agent_progress.success(
                self.AGENT_NAME,
                task.session_id,
                detail=message or 'Operación de escritorio completada',
                extra={'intent': getattr(task, 'intent', None)}
            )
        else:
            agent_progress.error(
                self.AGENT_NAME,
                task.session_id,
                detail=message or 'Operación de escritorio sin confirmación',
                extra={'intent': getattr(task, 'intent', None)}
            )
        return result
